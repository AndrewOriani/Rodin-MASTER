...
class HP34970A(SerialInstrument):
    ...
    def get_temp(self, ch, card=1):
        if type(ch)!=list:
            ch=[ch]
        else:
            pass
        val=self.c_query("MEAS:TEMP? (%s)"%(self.ch_list(ch, card)), bit_len=len(ch)*16+1)
        return self.val_unpack(val)
    
    def conf_temp_tcouple(self, ch, t_type='K', card=1):
        type_list=['B', 'E', 'K', 'J', 'N', 'R', 'S', 'T']
        if t_type in type_list:
            self.write("CONF:TEMP (%s)"%(self.ch_list(ch, card)))
            self.write("CONF:TEMP:TCouple, (%s)"%(self.ch_list(ch, card)))
            self.write("TEMP:TRAN:TCouple:TYPE %s, (%s)"%(t_type, self.ch_list(ch, card)))
        else:
